...
| Method | Returns | Caller |
|---|---|---|
| `listForOwner(ownerId, tx?, { scope? })` | `Person[]` | `/api/people` GET active/default and archived view |
| `listWithRelations(ownerId, tx?, { scope? })` | `PeoplePayload` | `/api/people` GET — single batched query producing the full payload |
