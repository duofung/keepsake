...
the business contact create/read/maintenance/archive-restore foundation for People,
...
├── people-maintenance/
│   ├── index.server.ts       ← current: request validation + mock/db dispatcher
│   ├── mock.server.ts        ← current: mock update/archive/restore over people
│   └── db.server.ts          ← current: DB-backed PeopleRepository.update/archive/restore
├── delivery-history/
│   ├── index.server.ts       ← current: mock/db dispatcher
│   ├── mock.server.ts        ← current: mock fallback
...
| `people-maintenance/index.server.ts` | `PATCH /api/people/[id]`, `POST /api/people/[id]/archive`, `POST /api/people/[id]/restore`, People drawer maintenance loop | Validates lightweight business contact updates (`name`, `segment`, `organization`, `roleTitle`, `sourceContext`, remember note, `lastContactAt`, `nextFollowUpAt`, `starred`) plus archive/restore requests, dispatches by `KEEPSAKE_DATA_SOURCE`, and returns the updated `Person` shape. Invalid body/field shapes map to 400 before the repository; already-active restore maps to 400. | Future richer contact maintenance can stay here; draft/delivery/channel contracts do not change. | `pnpm test:people`, `pnpm test:db:people`, `pnpm test:db:people-route` |
| `people-maintenance/db.server.ts` | `people-maintenance/index.server.ts` | `currentUserIdOrThrow()` + `transaction(ownerId)` + `PeopleRepository.update/archive/restore(ownerId, personId, input)`; not-found and cross-owner rows map to 404; invalid state / unsupported DB values map to 400. No SQL in the route. | Same repository calls with real auth. | `pnpm test:db:people-route` |
| `people-maintenance/mock.server.ts` | `people-maintenance/index.server.ts` | Mutates the in-memory `people` row for local/mock smoke coverage; archive sets `archivedAt`, restore clears it, and mock People/Home reads filter archived rows while delivery history remains independent. | Removed when mock is gone. | `pnpm test:people` |
