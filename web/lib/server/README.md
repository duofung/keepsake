...
| `auth/require-session.server.ts` | `app/{,people,workspace,history,profile}/page.tsx`, `app/signin/page.tsx` | Page-level guard. `requireSessionUserOrRedirect(returnTo)` calls `currentSessionUserOrThrow()` (cookie-only — no `DEV_OWNER_*` fallback); unauthenticated → `redirect("/signin?returnTo=…")`; misconfigured → re-raises so Next surfaces a 500. Exports a strict `safeReturnTo()` that accepts only relative paths. The route / API / server-seam layer continues to use the permissive `currentUserOrThrow()`; the split is intentional. | UI logout swap-in goes here when it lands. | `pnpm test:auth` (test-signin.mjs: signin render + dev CTA gating + page redirects + misconfigured → 500) |
