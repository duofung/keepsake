...
>   next follow-up, and soft archive, but there is still no standalone account
>   entity.
...
### `PATCH /api/people/[id]` and `POST /api/people/[id]/archive`
...
client                         server
  ...
  │                              │               → PeopleRepository.update/archive
  │                              ▼
  │  ◄──── Person / { person } ──┤  200, 400 invalid_request, 404 not_found
```

These routes are the People drawer maintenance loop only. They keep the
existing `/api/people` create/read contract separate and do not touch drafts,
deliveries, worker, webhook, Gmail, Telegram, or WhatsApp contracts.
...
| Repository implementations | `lib/repositories/catalog.server.ts`, `lib/repositories/people.server.ts`, `lib/repositories/drafts.server.ts`, `lib/repositories/deliveries.server.ts`, `lib/repositories/gmail-accounts.server.ts`, `lib/repositories/channel-accounts.server.ts` | Postgres implementations for catalog, people/occasion reads, person create/update/archive including encrypted business contact fields and soft archive filtering, message draft persistence/cache, delivery history reads, delivery enqueue/worker/webhook status updates, Gmail account metadata/token storage, and command-channel account identity. Occasion writes remain future work. | no | yes | no |
...
| `lib/server/remaster-overview/index.server.ts` | Composes `getPeoplePayload()` and `getDeliveryHistory()` into derived `RemasterDashboardOverview` accounts, contacts, activities, and lightweight touchpoint labels. The overview now carries business contact segment/organization/title/source fields, `lastTouchLabel`, `nextFollowUpLabel`, and `touchpointSummary` while preserving the legacy `relationshipType` compatibility field for Home/Workspace/History. Home consumes the overview directly as a follow-up dashboard and prefers `Person.nextFollowUpAt` when present; People consumes a compatibility view that carries the selected account into the relationship dossier drawer while retaining the legacy payload for person/culture/occasion details and Add contact / maintenance continuity; Workspace consumes the same compatibility view for account/contact/activity framing while keeping legacy person/occasion draft compatibility; History consumes a compatibility view that carries existing `Delivery[]` rows for touchpoint-timeline framing. Storage stays person-centered underneath. | Later replace the derivation with native account/contact/activity storage. |
...
| `lib/server/people-payload/db.server.ts` | `getDbPeoplePayload()` resolves dev owner, opens transaction, calls `PeopleRepository.listWithRelations(ownerId)`, which filters `archived_at IS NULL` by default. | Real auth replaces `auth/current-user.server.ts`; repository call remains. |
...
| `lib/server/people-maintenance/index.server.ts` | Validates drawer maintenance input (`name`, `segment`, `organization?`, `roleTitle?`, `sourceContext?`, `note?/notes?`, `lastContactAt?`, `nextFollowUpAt?`, `starred?`) and dispatches update/archive to mock or DB. Invalid field shapes return 400 before any repository call. | Same seam with real auth; future richer validation can stay here without changing routes. |
| `lib/server/people-maintenance/db.server.ts` | Resolves owner, opens `transaction(ownerId, …)`, calls `PeopleRepository.update` or `PeopleRepository.archive`, maps repo not-found to 404, and maps unsupported DB field values to 400. No SQL in routes. | Real auth replaces `auth/current-user.server.ts`; repository call remains. |
| `lib/server/people-maintenance/mock.server.ts` | Mutates the process-local mock `people` array for update/archive so smoke tests and local preview do not crash. Archive sets `archivedAt`; mock payload helpers hide archived people by default. | Deleted when mock is gone. |
...
| `lib/server/people-maintenance/index.server.ts` | Keep as dispatcher until mock can be deleted | `PATCH /api/people/[id]` and `POST /api/people/[id]/archive` request/error shapes; successful update returns `Person`, archive returns `{ person }` | `pnpm test:people`, `pnpm test:db:people`, `pnpm test:db:people-route` |
| `lib/server/people-maintenance/db.server.ts` | Real auth-backed owner resolution instead of `DEV_OWNER_ID` | `PeopleRepository.update/archive(ownerId, personId, input)` inside one transaction; cross-owner rows map to 404 | `pnpm test:db:people-route` |
