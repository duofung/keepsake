...
| API routes | `app/api/people/route.ts`, `app/api/drafts/route.ts`, `app/api/drafts/versions/route.ts` | Parse/return JSON and delegate. `/api/people` and draft routes can be mock- or DB-backed behind `KEEPSAKE_DATA_SOURCE`; `/api/drafts` still uses the mock generator for POST and has DB latest/version read paths for GET. | yes | people + draft persistence/cache/latest/version reads in DB mode | no |
...
6. **`GET /api/drafts/versions` request shape** = query params
...
7. **Culture rules resolve server-side only.** The client never sends a
   ...
   `KEEPSAKE_DATA_SOURCE=db`.
8. **`MessageDraft.paragraphs[].text` is plain text.** Highlights live in
...
9. **Server-only modules must begin with `import "server-only"`.** Filename
