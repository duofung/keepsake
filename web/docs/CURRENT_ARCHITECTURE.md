...
3. **`GET /api/session` response shape** = `{ user: { id, email, name, initials } }`.
   ...
   preserve. Covered by `pnpm test:auth`.
...
|---|---|---|
| `lib/server/auth/current-user.server.ts` | Resolves `{ id, email, name, initials }` and `OwnerId` from validated `DEV_OWNER_*` env. This is the only owner resolver; DB helpers keep calling `currentUserIdOrThrow()`, while `/api/session` calls `currentUserOrThrow()`. | Real session cookie / OAuth verification inside this module only. `/api/session` keeps returning `{ user }`; DB helper call sites keep their owner-id contract. |
...
|---|---|---|---|
| `lib/server/auth/current-user.server.ts` | Real auth-backed current user resolver | `currentUserIdOrThrow(): OwnerId`; `currentUserOrThrow()` returning `{ id, email, name, initials }`; typed unauthenticated vs misconfigured errors | `pnpm test:auth`, `pnpm test:boundaries` |
