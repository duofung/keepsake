...
### Home / People review bridge

Home review cards and the hero dossier CTA are ordinary page links to
`/people?review=<personId>`. People reads that query on the client and opens
the existing relationship dossier drawer only when the target is present in
the active people payload. Archived or missing contacts are ignored by this
bridge, so they do not re-enter the active review queue. From the drawer, the
user can use the existing follow-up actions or jump to `/workspace?person=...`
for drafting. This is UI routing only: no new API route, server contract,
schema, draft, delivery, worker, webhook, Gmail, Telegram, or WhatsApp behavior
is introduced.
