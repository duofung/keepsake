...
| `lib/server/oauth/gmail.server.ts` | Real Gmail OAuth service | `startGmailOAuth` and `completeGmailOAuth` result union; 400 invalid/provider-denied callbacks; 501 until configured; account persistence only through future `GmailAccountRepository`; no send/enqueue behavior | `pnpm test:oauth` |
