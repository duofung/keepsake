...
>   next follow-up, soft archive, and restore, but there is still no
...
### `PATCH /api/people/[id]`, `POST /api/people/[id]/archive`, and `POST /api/people/[id]/restore`
...
These routes are the People drawer maintenance loop only. Active contacts can
be updated or archived; archived contacts can be restored. They keep the
existing `/api/people` create/read payload shape separate and do not touch
...
| Domain | `lib/domain.ts` | Canonical TypeScript types — the contract between layers and over the wire. `Person` carries the business contact foundation (`segment`, `organization`, `roleTitle`, `sourceContext`) plus `lastContactAt`, `nextFollowUpAt`, `archivedAt`, and legacy relationship/culture fields. No HTML in message content. Card/icon hints are explicit structured fields, not rendered markup. | no | no | no |
