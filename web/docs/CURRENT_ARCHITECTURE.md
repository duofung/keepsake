...
>   `OccasionNode`, drafts, and deliveries. `Person` now carries the first
>   business contact fields (`segment`, `organization`, `roleTitle`,
>   `sourceContext`) plus lightweight maintenance fields for last touch,
>   touchpoint type, next follow-up, soft archive, restore, and a derived
>   follow-up rhythm classification (`overdue`, `today`, `this_week`, `later`,
>   `unscheduled`). Home and People now add a page-only review-to-action bridge
>   through `/people?review=<personId>` for active-contact dossier opening, but
>   there is still no standalone account entity.
...
client                    server
  ...
  │                         │           ▼
  │                         │         PeopleRepository.listWithRelations(ownerId, { scope })
  │                         │      ▼
...
source is still mock unless `KEEPSAKE_DATA_SOURCE=db` is set. Without query
params it returns active contacts only; `?view=archived` returns the same
`PeoplePayload` shape for archived contact management.

### People maintenance and follow-up actions
...
client                                  server
  ...
  │  POST /api/people/:id/follow-up       │
  │  POST /api/people/:id/follow-up/done  │
  │  POST /api/people/:id/follow-up/snooze│
  │  POST /api/people/:id/touchpoints     │
  ├──────────────────────────────────────►│
  │                                       │  app/api/people/[id]/*
  │                                       │      │ parse + auth
  │                                       │      ▼
  │                                       │  lib/server/people-maintenance/index.server.ts
  │                                       │      ├─ mock → people-maintenance/mock.server.ts
  │                                       │      └─ db   → transaction(ownerId)
  │                                       │               → PeopleRepository.update/archive/restore
  │                                       │               → PeopleRepository.setNextFollowUp
  │                                       │               → PeopleRepository.markFollowUpDone
  │                                       │               → PeopleRepository.snoozeFollowUp
  │                                       │               → PeopleRepository.logTouchpoint
  │                                       ▼
  │  ◄──────── Person / { person } ───────┤  200, 400 invalid_request, 404 not_found
```

These routes are the People drawer maintenance/action loop only. Active
contacts can be updated, archived, restored, marked done, snoozed, scheduled
for a next follow-up, or given a manual touchpoint label. The action routes
write only the existing `people` row (`last_contact_at`, `last_touchpoint_type`,
`next_follow_up_at`) and keep the existing `/api/people` create/read payload
shape separate. They do not touch drafts, deliveries, worker, webhook, Gmail,
Telegram, WhatsApp, or delivery history contracts.

### Home relationship intelligence / People review bridge

Home is a page-only Relationship Intelligence surface derived from the existing
compatibility overview. It groups active profiles, chooses one display issue per
relationship (`Needs context`, `Going quiet`, `Moment coming up`, `Ready to
draft`, or steady), and offers only lightweight `Open profile` and
`Draft outreach` actions. `Open profile` links to `/people?review=<personId>`.
People reads that query on the client and opens the existing relationship
dossier drawer only when the target is present in the active people payload.
Archived or missing contacts are ignored by this bridge, so they do not re-enter
the active review queue. From the drawer, the user can use the existing
follow-up actions or jump to `/workspace?person=...` for drafting. This is UI
routing only: no new API route, server contract, schema, draft, delivery,
worker, webhook, Gmail, Telegram, or WhatsApp behavior is introduced.
...
`/`). Unauthenticated visitors see a minimal ReMaster-framed entry point for
an account/contact outreach workspace. Google is only the identity provider
here; Gmail sender setup stays a later Profile action. The page has one CTA:
...
form lives at the bottom of the PLAN & PRIVACY section in
...
the client cannot override `relationship`, `cultureRule`, or `tone`.
Workspace outreach presets are page-layer framing only: when the user requests
a revision, the selected intent is folded into the same `userInstruction`
string instead of adding fields to this route. Default mock mode is unchanged
and does not write DB rows. DB mode resolves context
...
browser                   server
  ...
  │                         │      │
  │                         │      ▼
  │                         │  lib/server/remaster-overview/index.server.ts
  │                         │      │   getRemasterHistoryCompatibilityView()
  ...
  │                         │      ▼
  │  ◄──── History HTML ────┤  render relationship touchpoint timeline
  │                         │
```

History is a server component page, not an API route. It now asks the ReMaster
compatibility seam for an account/contact/activity overview plus existing
`Delivery[]` rows, then renders those rows as a relationship touchpoint
timeline. Delivery rows remain the source of truth for sent/opened/failed
history; the ReMaster read model only adds touchpoint labels and summaries so
the page can frame them as ongoing relationship activity. DB mode is still
read-only from the page perspective: delivery rows are read under RLS, while
enqueue, worker, webhook, and send contracts stay behind their existing seams.

The page itself does not import `lib/mock.ts`, repositories, SQL helpers, DB
clients, or the delivery-history dispatcher directly. The dispatcher still
lives behind `lib/server/delivery-history/index.server.ts`, which the
compatibility seam calls to get the existing delivery rows.
...
channels rather than mobile clients. They let users ask for account/contact
follow-ups, request outreach drafts, revise tone, and receive reminders from
the phone, while ReMaster web remains the **execution + review** surface for
editing, account setup, and final send confirmation.
...
interface CommandResponse {
  ...
  reviewUrl?:       string;           // relative ReMaster URL for review/action
}
```
...
ReMaster web. The `reviewUrl` is a relative URL that adapters may render
...
#         text: "I organized the outreach request. Review and send it in ReMaster.",
...
- Reply: the adapter calls Telegram Bot API `sendMessage` with the
  response text plus an absolute ReMaster review URL. The JSON response
...
`wa_id`/phone, or Slack user/team/channel id links to an app
...
|---|---|---|---|---|---|
| Pages | `app/page.tsx`, `app/people/`, `app/workspace/`, `app/history/`, `app/profile/`, `app/signin/` | Render. Home, People, Workspace, and History call the ReMaster compatibility overview seam, which derives account/contact/activity/touchpoint cards from people payload + delivery history while storage remains `Person`-centered; Home now frames the same overview as a minimal Relationship Intelligence surface with profile overview, priority relationship issues, and recent relationship signals / next actions. Home derives one display issue per relationship (`Needs context`, `Going quiet`, `Moment coming up`, `Ready to draft`, or steady), bridges `Open profile` to `/people?review=<personId>`, and keeps `Draft outreach` on `/workspace?person=<primaryContactId>` while default dashboard activity only considers active contacts; People uses native contact `segment` fields for business tabs (`All`, `Clients`, `Partners`, `Prospects`, `Investors`, `Personal`), adds an Active / Archived management toggle plus an active-only All active / Needs attention review layer above the tabs, consumes `/people?review=<personId>` for active-contact dossier opening, shows organization / role title, next follow-up, last touch, rhythm, and business context, passes the selected compatibility account plus legacy person/culture/occasion payload into a business-first relationship dossier drawer with overview, inline maintenance for active contacts, review-only archived state, restore/archive actions, relationship context, touchpoints, rhythm summary, notes, follow-up actions, and workspace drafting actions, posts the business Add contact form to `/api/people`, patches, archives, restores, and runs follow-up actions on contacts through the maintenance routes, preserves mock-mode `local-*` browser continuity, and keeps Workspace links on `/workspace?person=<primaryContactId>` for active contacts; Workspace receives the same legacy payload only for existing draft context, uses the overview plus `Person.segment` to frame the surface as client follow-up / partner outreach / prospect outreach / investor update / personal note, and adds local intent presets (`Follow up`, `Recap`, `Check-in`, `Congratulations`, `Intro`, `Personal`) that adjust helper copy, preset quick actions, and the existing `userInstruction` text without adding backend fields; draft restore/generate/version interactions stay behind `/api/drafts`, autosave stays behind `PATCH /api/drafts`, and the send buttons still POST to `/api/deliveries` for queue-boundary enqueue after flushing pending edits; History receives the same overview plus the existing `Delivery[]` rows and frames them as a relationship touchpoint timeline while preserving delivery status rendering, including rows for archived contacts; Profile and Sign-in are ReMaster-framed account/contact/outreach entry surfaces on top of the current auth, Gmail, and command-channel seams, so Profile keeps Gmail connect/disconnect, sign-out, and channel link forms while Sign-in keeps Google/dev CTA and returnTo behavior. | yes (People create/update/archive/restore/follow-up actions, Workspace draft fetches + autosave PATCH + delivery enqueue, Profile forms, Sign-in auth links) | via server helper when DB mode is enabled | no |
| API routes | `app/api/session/route.ts`, `app/api/auth/signout/route.ts`, `app/api/oauth/gmail/*/route.ts`, `app/api/people/route.ts`, `app/api/people/[id]/route.ts`, `app/api/people/[id]/archive/route.ts`, `app/api/people/[id]/restore/route.ts`, `app/api/drafts/route.ts`, `app/api/drafts/versions/route.ts`, `app/api/deliveries/route.ts`, `app/api/gmail/disconnect/route.ts`, `app/api/webhooks/deliveries/route.ts`, `app/api/channels/mock/route.ts`, `app/api/channels/mock/inbound/route.ts`, `app/api/channels/mock/link/route.ts`, `app/api/channels/mock/revoke/route.ts`, `app/api/channels/telegram/route.ts`, `app/api/channels/telegram/link/route.ts`, `app/api/channels/telegram/revoke/route.ts`, `app/api/channels/whatsapp/route.ts`, `app/api/channels/whatsapp/revoke/route.ts` | Parse/return JSON and delegate. `/api/session` exposes the stable `{ user }` contract and maps auth errors; `/api/auth/signout` clears `keepsake_session` and 303s to `/signin` (no DB, no Google revoke, no Gmail disconnect); Gmail OAuth routes own the connect flow; `/api/people` GET returns active `PeoplePayload` by default, accepts `?view=archived` for archived management, and POST creates a `Person` through the people-create seam, now accepting business contact fields while still filling legacy relationship/culture defaults internally; `PATCH /api/people/[id]`, `POST /api/people/[id]/archive`, and `POST /api/people/[id]/restore` are thin maintenance routes over the people-maintenance seam and return 400 invalid request / 404 not found for invalid state/body or cross-owner edits; draft routes can be mock- or DB-backed behind `KEEPSAKE_DATA_SOURCE`; `/api/drafts` POST swaps between mock and LLM behind `KEEPSAKE_DRAFT_SOURCE` (default mock) and PATCH persists Workspace subject + body + card edits as new canonical draft versions; `/api/deliveries` is the send-boundary contract that returns 202 `QueuedDelivery` without calling Gmail; `/api/webhooks/deliveries` is the provider-agnostic delivery-status callback (shared-secret gate, never reads current user); `/api/channels/mock` exercises the pure router; `/api/channels/mock/inbound` exercises DB-backed provider identity resolution and returns a review pointer only; `/api/channels/mock/{link,revoke}` and `/api/channels/telegram/{link,revoke}` are owner-scoped Profile mutations that manage `channel_accounts` rows inbound routes then resolve against; `/api/channels/whatsapp/revoke` revokes a linked WhatsApp channel row from Profile; `/api/channels/telegram` consumes Telegram webhook updates and `/start <token>` link commands; `/api/channels/whatsapp` consumes DB-only WhatsApp text webhook payloads behind a shared-secret header, including signed ReMaster link-token messages, and returns the same review-first JSON response. The ReMaster compatibility overview is page-only for Home + People + Workspace + History; Profile / Sign-in and command-channel replies are ReMaster-framed surfaces on top of the same auth, Gmail, and channel contracts. This does not change `/api/drafts`, `/api/deliveries`, auth, Gmail, worker, delivery webhook, or send contracts. | yes | people create/read/update/archive/restore/follow-up actions + draft persistence/cache/latest/version reads + edited-version inserts, gmail-account read for sender precondition, delivery enqueue in DB mode only, delivery status updates from webhook in DB mode only, channel-account lookup/link/revoke for mock/Telegram/WhatsApp in DB mode only | optional via `KEEPSAKE_DRAFT_SOURCE=openai` |
| Server services | `lib/server/remaster-overview/index.server.ts`, `lib/server/people-payload/{index,db,mock}.server.ts`, `lib/server/people-create/{index,db,mock}.server.ts`, `lib/server/people-maintenance/{index,db,mock}.server.ts`, `lib/server/draft-service/{index,db,mock,generator-errors}.server.ts`, `lib/server/draft-context/{index,db,mock}.server.ts`, `lib/server/draft-generator/{index,mock,openai}.server.ts`, `lib/server/delivery-history/{index,db,mock}.server.ts`, `lib/server/delivery-send/{index,db,mock,types}.server.ts`, `lib/server/auth/current-user.server.ts`, `lib/server/oauth/gmail.server.ts`, `lib/server/db/transaction.server.ts`, `lib/server/crypto/envelope.server.ts` | Server-only orchestration. `auth/current-user` is the only current-user / owner resolver; `oauth/gmail` owns the Gmail provider boundary; people payload/create/maintenance, drafts, draft context, delivery history, and delivery send are DB-capable runtime verticals; `people-payload` accepts active/archived read scope while defaulting active; `people-maintenance` validates drawer update/archive/restore/follow-up inputs and delegates to mock helpers or `PeopleRepository.update/archive/restore/setNextFollowUp/markFollowUpDone/snoozeFollowUp/logTouchpoint` inside the existing transaction seam; `remaster-overview/index.server.ts` is the compatibility migration seam for Home + People + Workspace + History, composing active people payload + delivery history into derived account/contact/activity read models, `followUpRhythm`, and sorted `reviewAccounts`, while returning the legacy payload only where People still needs drawer/add/edit compatibility and Workspace still needs person/occasion draft compatibility; History gets delivery rows through the same seam only for activity-timeline framing; Profile / Sign-in framing stays in the pages and existing auth/channel/Gmail seams, with no new compatibility seam; `draft-generator/index.server.ts` dispatches `mock` vs `openai` behind `KEEPSAKE_DRAFT_SOURCE` so the route contract stays unchanged; `delivery-send` is the queue boundary (validate → ownership → sender precondition → latest draft → enqueue, no Gmail call). | no | yes in DB mode | optional via `KEEPSAKE_DRAFT_SOURCE=openai` |
| Mock store | `lib/mock.ts` | In-memory data: 5 people with business contact segments, 7 occasions, 4 cultures, 5 relationships, 4 deliveries + finder helpers. Active people payloads and occasion helpers filter `archivedAt`; archived payloads opt into archived rows; delivery history remains independent. | no | no | no |
| Domain | `lib/domain.ts` | Canonical TypeScript types — the contract between layers and over the wire. `Person` carries the business contact foundation (`segment`, `organization`, `roleTitle`, `sourceContext`) plus `lastContactAt`, `lastTouchpointType`, `nextFollowUpAt`, `archivedAt`, and legacy relationship/culture fields. No HTML in message content. Card/icon hints are explicit structured fields, not rendered markup. | no | no | no |
| Presentation | `lib/presentation.ts` | Maps `OccasionKind`/`Tone`/`Channel` → icon names, gradients, chip text. UI only. | no | no | no |
| Repository implementations | `lib/repositories/catalog.server.ts`, `lib/repositories/people.server.ts`, `lib/repositories/drafts.server.ts`, `lib/repositories/deliveries.server.ts`, `lib/repositories/gmail-accounts.server.ts`, `lib/repositories/channel-accounts.server.ts` | Postgres implementations for catalog, people/occasion reads, person create/update/archive/restore including encrypted business contact fields, active/archived read scopes, message draft persistence/cache, delivery history reads, delivery enqueue/worker/webhook status updates, Gmail account metadata/token storage, and command-channel account identity. Occasion writes remain future work. | no | yes | no |
| DB scripts | `db/schema.sql`, `db/seed_catalog.sql`, `scripts/seed-dev-fixtures.mjs` | Postgres 17 schema + catalog seed + encrypted local-dev fixture seed. `people` includes business contact columns, `next_follow_up_at`, and `archived_at`; active People/Home reads hide archived rows while delivery history keeps denormalized recipient rows. | no | yes (manual/dev) | no |
...
1. **`lib/domain.ts` shapes** — `Person`, `ContactSegment`, `OccasionNode`, `MessageDraft`,
...
2. **`GET /api/people` response shape** = `PeoplePayload`, with each `Person`
   carrying `segment`, `organization`, `roleTitle`, and `sourceContext`.
   Covered by `pnpm test:people`, which also smokes the People account/contact
...
6. **`POST /api/drafts` request shape** = `{ personId, occasionId, userInstruction }`,
   ...
   in DB prompt hashing. Workspace's business outreach presets are encoded
   only as prose inside `userInstruction` when the user requests a revision;
   there is no `intent` / `preset` field. Covered by `pnpm test:drafts` and
   the Workspace page smoke.
...
|---|---|---|
| `lib/server/auth/current-user.server.ts` | Resolves `{ id, email, name, initials, sendingAccount }` and `OwnerId` from the signed `keepsake_session` cookie first, with `DEV_OWNER_*` fallback only when no cookie is present. Mock mode returns `sendingAccount: null`; DB mode reads the owner's primary Gmail account. This is the only owner resolver; `/api/session`, Home, Workspace, Profile, and Sign-in keep using the same auth/session shape even though the visible Profile / Sign-in copy is now ReMaster-framed. | Real session cookie / OAuth verification hardening stays inside this module only. `/api/session` keeps returning `{ user }`; Home, Workspace, Profile, and Sign-in keep rendering the same identity/redirect shape; DB helper call sites keep their owner-id contract. |
...
| `lib/server/remaster-overview/index.server.ts` | Composes active `getPeoplePayload()` and `getDeliveryHistory()` into derived `RemasterDashboardOverview` accounts, contacts, activities, lightweight touchpoint labels, and follow-up rhythm. The overview now carries business contact segment/organization/title/source fields, `lastTouchLabel`, `nextFollowUpLabel`, `touchpointSummary`, `followUpRhythm`, `reviewAccounts`, and `reviewQueueCount` while preserving the legacy `relationshipType` compatibility field for Home/Workspace/History. Rhythm classification prefers `Person.nextFollowUpAt`, falls back to the primary occasion date/window, and groups accounts as `overdue`, `today`, `this_week`, `later`, or `unscheduled`. Home consumes the overview directly as Relationship Intelligence display data, deriving profile overview groups, priority relationship issues, recent signals, `/people?review=<personId>` profile links, and `/workspace?person=<personId>` draft links while excluding deliveries tied to non-active people from default recent-outreach cards; People consumes a compatibility view that carries the selected account into the relationship dossier drawer while retaining the legacy payload for person/culture/occasion details and Add contact / maintenance continuity; Workspace consumes the same compatibility view for account/contact/activity framing while keeping legacy person/occasion draft compatibility; History consumes a compatibility view that carries existing `Delivery[]` rows for touchpoint-timeline framing. Storage stays person-centered underneath. | Later replace the derivation with native account/contact/activity storage. |
| `lib/server/people-payload/index.server.ts` | Dispatches to mock by default, or DB when `KEEPSAKE_DATA_SOURCE=db`; accepts an optional `view` of `active` or `archived`, defaulting active. | Later auth replaces `DEV_OWNER_ID`; route/page imports stay the same. |
| `lib/server/people-payload/mock.server.ts` | `getMockPeoplePayload({ view })` reads `peoplePayload(view)` from `lib/mock.ts`. | Kept as fallback until all runtime paths are DB-backed. |
| `lib/server/people-payload/db.server.ts` | `getDbPeoplePayload({ view })` resolves dev owner, opens transaction, calls `PeopleRepository.listWithRelations(ownerId, tx, { scope })`, which filters `archived_at IS NULL` by default and `archived_at IS NOT NULL` for archived management. | Real auth replaces `auth/current-user.server.ts`; repository call remains. |
| `lib/server/people-create/index.server.ts` | Validates Add contact input (`name`, `segment`, `organization?`, `roleTitle?`, `sourceContext?`, `note?`, `starred?`), derives avatar/known-fact defaults, fills legacy relationship/culture defaults when the business-first form omits them, and dispatches to mock or DB. Mock returns a `local-*` `Person` for browser-local preview persistence; DB writes through `PeopleRepository.create`. | Keep as create-only seam; edit/archive live in `people-maintenance`. |
...
| `lib/server/people-maintenance/index.server.ts` | Validates drawer maintenance input (`name`, `segment`, `organization?`, `roleTitle?`, `sourceContext?`, `note?/notes?`, `lastContactAt?`, `nextFollowUpAt?`, `starred?`) and dispatches update/archive/restore to mock or DB. Invalid field shapes return 400 before any repository call; restoring an already-active DB row maps to 400. | Same seam with real auth; future richer validation can stay here without changing routes. |
| `lib/server/people-maintenance/db.server.ts` | Resolves owner, opens `transaction(ownerId, …)`, calls `PeopleRepository.update`, `PeopleRepository.archive`, or `PeopleRepository.restore`, maps repo not-found to 404, repo validation to 400, and unsupported DB field values to 400. No SQL in routes. | Real auth replaces `auth/current-user.server.ts`; repository call remains. |
| `lib/server/people-maintenance/mock.server.ts` | Mutates the process-local mock `people` array for update/archive/restore so smoke tests and local preview do not crash. Archive sets `archivedAt`, restore clears it, and mock payload helpers hide archived people by default unless the archived view is requested. | Deleted when mock is gone. |
| `lib/server/delivery-history/index.server.ts` | Dispatches to mock by default, or DB when `KEEPSAKE_DATA_SOURCE=db`. | Later auth replaces `DEV_OWNER_ID`; History reaches this only through the ReMaster compatibility seam. |
...
| `lib/server/remaster-overview/index.server.ts` | Keep as the compatibility seam while storage is still person-centered | `getRemasterDashboardOverview()` signature, Home's account/contact/activity framing, People's `primaryContactId` bridge to dossier review and `/workspace?person=...`, Workspace's account outreach framing without changing draft/send APIs, and History's activity-timeline framing without changing delivery/webhook/worker contracts | `pnpm test:home`, `pnpm test:people`, `pnpm test:workspace`, `pnpm test:history` |
| `lib/server/people-payload/index.server.ts` | Keep as dispatcher until mock can be deleted | `getPeoplePayload({ view? })`; `GET /api/people` returning active `PeoplePayload` by default and archived `PeoplePayload` only for `?view=archived` | `pnpm test:people`, `pnpm test:db:people-route` |
| `lib/server/people-payload/db.server.ts` | Real auth-backed owner resolution instead of `DEV_OWNER_ID` | Repository call and `PeoplePayload` shape, including active/archived scope | `pnpm test:db:people-route` |
...
| `lib/server/people-maintenance/index.server.ts` | Keep as dispatcher until mock can be deleted | `PATCH /api/people/[id]`, `POST /api/people/[id]/archive`, and `POST /api/people/[id]/restore` request/error shapes; successful update returns `Person`, archive/restore return `{ person }` | `pnpm test:people`, `pnpm test:db:people`, `pnpm test:db:people-route` |
| `lib/server/people-maintenance/db.server.ts` | Real auth-backed owner resolution instead of `DEV_OWNER_ID` | `PeopleRepository.update/archive/restore(ownerId, personId, input)` inside one transaction; cross-owner rows map to 404 | `pnpm test:db:people-route` |
| `lib/server/delivery-history/index.server.ts` | Keep as dispatcher until mock can be deleted | `getDeliveryHistory()` signature; `getRemasterHistoryCompatibilityView()` receives `Delivery[]`; email/post remain badges rather than separate product modes | `pnpm test:history`, `pnpm test:db:history-route` |
...
- **Don't treat People as a native CRM.** `POST /api/people` can create a
  person and the drawer can update/archive lightweight business contact fields,
  but there is still no account table, merge/import flow, deal pipeline,
  native activity schema, or reminder scheduler.
