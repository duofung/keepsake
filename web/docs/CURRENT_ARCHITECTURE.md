...
browser                   server
  ...
  │                         │      ▼
  │  ◄──── History HTML ────┤  render account/contact activity timeline
  │                         │
```
...
`Delivery[]` rows, then renders those rows as an activity timeline. DB mode is
still read-only from the page perspective: delivery rows are read under RLS,
while enqueue, worker, webhook, and send contracts stay behind their existing
seams.
...
| `lib/server/remaster-overview/index.server.ts` | Composes `getPeoplePayload()` and `getDeliveryHistory()` into derived `RemasterDashboardOverview` accounts, contacts, and activities. The overview now carries business contact segment/organization/title/source fields while preserving the legacy `relationshipType` compatibility field for Home/Workspace/History. Home consumes the overview directly; People consumes a compatibility view that also carries the legacy payload for drawer continuity; Workspace consumes the same compatibility view for account/contact/activity framing while keeping legacy person/occasion draft compatibility; History consumes a compatibility view that carries existing `Delivery[]` rows for activity-timeline framing. Storage stays person-centered underneath. | Later replace the derivation with native account/contact/activity storage. |
