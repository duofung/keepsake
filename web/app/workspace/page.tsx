...
function Workspace() {
  ...
  // Load people payload once
  useEffect(() => {
    let alive = true;
    fetch("/api/people")
      .then((r) => r.json() as Promise<PeoplePayload>)
      .then((p) => { if (alive) setPayload(p); })
      .catch(() => { /* surfaced via UI loading state */ });
    return () => { alive = false; };
  }, []);
  ...
  const relationship: Relationship | null = useMemo(
    () => (payload && person) ? payload.relationships.find((r) => r.id === person.relationshipId) ?? null : null,
    [payload, person],
  );
  const culture: CultureRule | null = useMemo(
    () => (payload && person) ? payload.cultures.find((c) => c.id === person.cultureId) ?? null : null,
    [payload, person],
  );
  const occasion: OccasionNode | null = useMemo(
    () => (payload && person?.nextOccasionId)
